 ...
 * `monitoringUntilMs` lives only on the personal follow — never the public catalog.
 */
...
export function monitoringUntilMsForPreset(preset, nowMs = Date.now()) {
  if (preset === 'forever') return null;
  if (preset === 'week') return nowMs + WEEK_MS;
  return nowMs + DAY_MS;
}

export function monitoringUntilMsForCustomHours(hours, nowMs = Date.now()) {
...
/** Mutate `bag.stations` when any timer has expired. Returns whether the bag changed. */
...
export function bagHasActiveStation(bag) {
  return (bag?.stations || []).some((s) => s.enabled !== false && String(s.stationId || '').trim());
}
